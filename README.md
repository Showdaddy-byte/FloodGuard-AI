...
Verified end-to-end (fetch → cache → scoring → display) and confirmed correctly wired — this was already solidly built. It augments (not replaces) the existing weather/terrain/hydrology pipeline with:

| Dataset | What it adds |
|---|---|
| Copernicus DEM (`COPERNICUS/DEM/GLO30`) | Elevation + slope, used as a fallback when Open-Meteo's elevation lookup is unavailable |
| Sentinel-1 SAR (`COPERNICUS/S1_GRD`) | Automatic flood extent detection — compares recent radar backscatter against a pre-event baseline, excludes permanent water bodies and steep terrain to reduce false positives |
| Sentinel-2 (`COPERNICUS/S2_SR_HARMONIZED`) | NDWI (surface water index) and NDVI from recent low-cloud imagery |
| JRC Global Surface Water (`JRC/GSW1_4/GlobalSurfaceWater`) | Historical water occurrence — how often a location has been water-covered |
| CHIRPS (`UCSB-CHG/CHIRPS/DAILY`) | Satellite-derived 7-day and 30-day rainfall accumulation, cross-validating ground-station rainfall |
| Dynamic World (`GOOGLE/DYNAMICWORLD/V1`) | Near-real-time land cover classification (built area, flooded vegetation, water, etc.) |

All of this feeds into both the flood score (`_context_bonus`) and a dedicated "Earth Engine Intelligence" panel on the result page — not just a black-box number.
...
- **Request latency**: each Earth Engine lookup makes several sequential calls to Google's servers and can take several seconds to over a minute on a cold cache. Results are cached for `EARTH_ENGINE_CONTEXT_TTL_HOURS` (6h) per location to keep this affordable; the *first* search for a new location will be the slowest.
- **This was not tested against live Earth Engine from the environment this fix was written in** — no network access or credentials were available there. The wiring was verified by tracing the full code path (confirmed the score and factors are actually consumed, not just computed and discarded), but real-world verification against your actual GEE project happens when you check `/health` and run a real search after deploying.
